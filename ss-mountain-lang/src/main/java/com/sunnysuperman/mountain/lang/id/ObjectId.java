...
public final class ObjectId implements Comparable<ObjectId> {
	 ...
	 */
	public ObjectId() {
		this(System.currentTimeMillis(), MACHINE_IDENTIFIER, PROCESS_IDENTIFIER, NEXT_COUNTER.getAndIncrement(), false);
	}

	public ObjectId(final AtomicInteger counter) {
		this(System.currentTimeMillis(), MACHINE_IDENTIFIER, PROCESS_IDENTIFIER, counter.getAndIncrement(), false);
	}

	public ObjectId(final long timestamp) {
		this(timestamp, MACHINE_IDENTIFIER, PROCESS_IDENTIFIER, NEXT_COUNTER.getAndIncrement(), false);
	}

	public ObjectId(final long timestamp, final AtomicInteger counter) {
		this(timestamp, MACHINE_IDENTIFIER, PROCESS_IDENTIFIER, counter.getAndIncrement(), false);
	}

	/**
	 * Constructs a new instances using the given date, machine identifier, process
	 ...
	 */
	public ObjectId(final long timestamp, final int machineIdentifier, final short processIdentifier,
			final int counter) {
		this(timestamp, machineIdentifier, processIdentifier, counter, true);
	}

	private ObjectId(final long timestamp, final int machineIdentifier, final short processIdentifier,
